...
const textureLoader = new THREE.TextureLoader();
const nebuTexture = textureLoader.load('img/sun_glow.png');
const nebuMaterial = new THREE.SpriteMaterial({ map: nebuTexture});

const nebulaSprite = new THREE.Sprite(nebuMaterial);

nebulaSprite.material.side = THREE.DoubleSide;
nebulaSprite.scale.set(100, 100,);
...
function addStar() {
    ...
    const [x, y, z] = Array(3).fill().map(() => THREE.MathUtils.randFloatSpread(1000000));
